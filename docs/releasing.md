...
Inspect the tarball list and sizes, update `CHANGELOG.md`, then set the intended version with `npm version`. Only after CI passes should a maintainer run `npm publish`, push the version commit/tag, and create the matching GitHub release. Publishing and pushing are intentionally manual.
